...
{% if klass.has_nonoptional %}	
	...
	public convenience init({%- set ns = namespace(sep='') -%}{%- for nonop in klass.properties %}{% if nonop.nonoptional %}{{ns.sep}}{%- set ns.sep = ', ' -%}{{ nonop.name }}: {% if nonop.is_array %}[{% endif %}{{ nonop.class_name }}{% if nonop.is_array %}]{% endif %}
	{%- endif %}{% endfor -%}
	) {
		self.init(json: nil)
		...
		{%- for prop in klass.properties %}
			if let exist = js["{{ prop.orig_name }}"] {
				...
				if let val = exist as? {% if prop.is_array %}[{% endif %}{{ prop.json_class }}{% if prop.is_array %}]{% endif %} {
					...
					errors.append(FHIRJSONError(key: "{{ prop.orig_name }}", wants: {% if prop.is_array %}Array<{% endif %}{{ prop.json_class }}{% if prop.is_array %}>{% endif %}.self, has: Swift.type(of: exist)))
				}
			}
